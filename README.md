...
| `/recognize` | Identifies income from savings account movements — salary, cashback, IOF adjustments, investment yields, named transfers. Matches salary by amount + date window rules from `resources/income_memory.md`. Provisions expected salary for partial months. |
...
| `/classify` | Classifies both expenses and income in one pass. Uses `expenses_memory.md` for merchant patterns and `income_memory.md` for salary/income rules. Accepts transactions inline or from file. Updates both memory files with new patterns discovered. |
