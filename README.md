...
| **`/settle`** | Closes the previous month by removing all provisional/estimated rows and locking the final numbers. **Always re-fetches the month fresh** and confirms its movements are **complete** — every transaction present and **no `pending` left** (`PENDING credit transactions remaining: 0`). If pending rows persist after a few re-fetches, the bill is still open, so it **stops and warns** rather than finalizing incomplete data. Once complete, it strips provisionals (`--final`) and triggers `/heartbeat` for the current month. Best run at the start of each new month to finalize last month's budget. |
| **`/audit`** | Validates the schema of both raw files and compiled budget output. Auto-fixes common issues like encoding, missing fields, and malformed entries, retrying up to 3 times. Also enforces **installment continuity** — a plan that carried installment `k/N` in either of the last two months has its `k+1/N` successor provisioned this month when Pluggy's open bill omits it (open-bill installments are volatile). In `--final` mode (closed months, run by `/settle`) it also enforces that **no row remains `pending`** — any lingering pending charge is an error and gets normalized to `posted`. Called automatically at the end of `/compile`, `/heartbeat`, and `/settle` to ensure data integrity. |
